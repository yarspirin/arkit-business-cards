...
@property (weak, nonatomic) IBOutlet UIButton *hidePlanesButton;
@end
...
#warning Yaroslav
// Added button to view and customized it

- (void)setupHidePlanesButton {
    self.hidePlanesButton.layer.borderWidth = 1.0;
    self.hidePlanesButton.layer.borderColor = [UIColor whiteColor].CGColor;
    self.hidePlanesButton.layer.cornerRadius = 5.0;
}
- (IBAction)hidePlanesButtonTapped:(id)sender {
    [self hidePlanes];
}
...
- (void)setupScene {
  ...
  self.planes = [NSMutableDictionary new];
    ...
    // A list of all the cards being rendered in the scene
...
- (void)setupRecognizers {
  ...
  UITapGestureRecognizer *tapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(insertCardFrom:)];
...
#warning Yaroslav
// This method was for cube, now it's only for card

- (void)insertCardFrom: (UITapGestureRecognizer *)recognizer {
...
- (void)explode:(ARHitTestResult *)hitResult {
  ...
  for(SCNNode *cardNode in self.cards) {
    // The distance between the explosion and the geometry
    SCNVector3 distance = SCNVector3Make(
                                          cardNode.worldPosition.x - position.x,
                                          cardNode.worldPosition.y - position.y,
                                          cardNode.worldPosition.z - position.z
                                          );
    ...
    [[cardNode.childNodes firstObject].physicsBody applyForce:distance atPosition:SCNVector3Make(0.05, 0.05, 0.05) impulse:YES];
  }
}

- (void)insertCard:(ARHitTestResult *)hitResult {
  // We insert the geometry slightly above the point the user tapped, so that it drops onto the plane
  // using the physics engine
    
#warning Yaroslav
// Changed offset above plane (was 10.0)

  float insertionYOffset = 0.0;
  SCNVector3 position = SCNVector3Make(
                                       hitResult.worldTransform.columns[3].x,
    ...
    Card *card = [[Card alloc] initAtPosition:position withMaterial:[Card currentMaterial]];
    [self.cards addObject:card];
  [self.sceneView.scene.rootNode addChildNode:card];
}
